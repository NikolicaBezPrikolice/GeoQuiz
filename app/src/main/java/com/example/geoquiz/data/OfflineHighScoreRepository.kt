...
class OfflineHighScoreRepository(private val highScoreDao: HighScoreDao) : HighScoreRepository {
    ...
    override fun getScoreStream(name: String): Flow<HighScore?> = highScoreDao.getOneScore(name)

    override suspend fun updateScore(score: HighScore) =
        highScoreDao.updateScoreByName(score.name, score.score)
}